...
static int handle_rx(struct CTXTYPE *ctx, struct pkt_metadata *md) {
  ...
  if (!action) {
    pcn_log(ctx, LOG_ERR, "eBPF - egress: action not set -> DROP");
    return RX_DROP;
  }
  ...
  case PASS:
    ...
    return RX_OK;
  case SLOWPATH:
    pcn_log(ctx, LOG_INFO, "Egress: sending packet to slow path");
    return pcn_pkt_controller(ctx, md, SLOWPATH_REASON);
  default:
    pcn_log(ctx, LOG_DEBUG, "Egress: bad action %d", *action);
    return RX_DROP;
  }
